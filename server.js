...
// Helper to resolve Minecraft SRV records and fallback IPs
async function resolveMinecraftServer(host, port) {
    let targetHost = host;
    let targetPort = port;
    ...
    try {
        ...
        // SRV resolution failed or doesn't exist, fallback to direct host/port
    }
...
function createBot(username, host, port, password, proxyUrl, reconnectDelay, socket) {
    ...
    if (parsedProxy) {
        botOptions.connect = async (client) => {
            try {
                // 1. Resolve SRV record to get the exact backend target host and port
                const { targetHost, targetPort } = await resolveMinecraftServer(host, port);
                
                socket.emit('bot_log', { 
                    botId: username, 
                    text: `Resolved Target: ${targetHost}:${targetPort}. Opening SOCKS5 Tunnel...` 
                });
                ...
                // 2. Establish raw SOCKS5 socket stream
                ...
                socket.emit('bot_log', { botId: username, text: `Connection Setup Error: ${err.message}` });
            }
        };
    }
